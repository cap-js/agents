...
export default class AnthropicLLMService extends LLMService {
  ...
  async *_stream(rows) {
    ...
    const thinking = this.options?.thinking ?? cds.env.agents?.params?.thinking !== false
    const { baseURL = 'https://api.anthropic.com', apiKey } = this._config

    LOG.debug('Anthropic request', { model, thinking: !!thinking })

    let prefix = `{"model":${JSON.stringify(model)},"max_tokens":${max_tokens},"stream":true`
    if (thinking) {
      const thinkingParam = thinking === true
        ? { type: 'enabled', budget_tokens: Math.floor(max_tokens * 0.8) }
        : thinking
      prefix += `,"thinking":${JSON.stringify(thinkingParam)}`
    }
  ...
  async *_toAnthropicMessages(rows, prefix) {
    ...
    function* closeSystem() {
      if (pendingSystem) {
        pendingSystem.cache_control = EPHEMERAL
        yield sysSep + JSON.stringify(pendingSystem)
        pendingSystem = null
      }
      yield `]`
      inSystem = false
    ...
    for await (const row of rows) {
      ...
      if (row.role === 'system') {
        ...
        yield* openSystem()
        if (pendingSystem) { yield sysSep + JSON.stringify(pendingSystem); sysSep = ',' }
        pendingSystem = { type: 'text', text: row.content || '' }
        continue
      }
