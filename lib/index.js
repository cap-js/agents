  ...
  router.post("/", express.json({ limit: envelopeLimit }), async (req, res) => {
    ...
    if (span) {
      ...
      if (cds.env.agents?.mlflow && !res.headersSent) {
        ...
        // Propagate via span attribute to RoutingSpanProcessor
        const evalRunId = req.headers?.["x-eval-run-id"]
        if (evalRunId) span.setAttribute("mlflow.sourceRun", evalRunId)
      }
      if (cds.context) {
