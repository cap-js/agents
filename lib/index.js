...
import { mlflowAttrs, setSpanAttrs } from "./telemetry/mlflow/index.js"
...
export default function AgentProtocolAdapter(srv, options = {}) {
  ...
  router.use((req, res, next) => {
    ...
    if (requires != null) {
      ...
      roles = Array.isArray(requires) ? requires : [requires]
      if (roles.length === 0) roles = null // empty array = no restriction
    } else if (restrict != null) {
      // @restrict present — extract all 'to' roles; entries without 'to' mean "any role"
      const extracted = restrict
        .map((r) => r.to)
        .flat()
        .filter(Boolean)
      if (extracted.length > 0) {
        ...
        LOG.warn("@restrict on service has no 'to' roles — cannot enforce access control", {
          service: srv.name,
        })
        roles = null
      }
    } else {
      // No annotation — production fallback
      roles = process.env.NODE_ENV === "production" &&
    ...
    if (roles.some((role) => user.has(role))) return next()
    if (req.method === "POST") {
      const body = JSON.stringify({
        jsonrpc: "2.0",
        id: req.body?.id ?? null,
  ...
  router.post("/", express.json({ limit: "5mb" }), async (req, res) => {
    ...
    if (span) {
      ...
      if (cds.env.agents?.mlflow && !res.headersSent) {
        try {
          res.setHeader("x-trace-id", span.spanContext().traceId)
        } catch {
          /* headers already sent by an upstream middleware — safe to ignore */
        }
      }
    }
      ...
      if (maxLen > 0 && text?.length > maxLen) {
        ...
        if (!res.headersSent) {
          res.status(400).json({
            jsonrpc: "2.0",
            // REVISIT: i18n
            error: { code: -32029, message: `The message must not exceed ${maxLen} characters` },
            id: req.body?.id || null,
          })
        }
        return
    ...
    try {
      ...
      if (typeof result?.[Symbol.asyncIterator] === "function") {
        ...
        res.flushHeaders()
        // Release generator resources and abort execution when client disconnects.
        // Replace the generic abortOnClose with one that also returns the generator.
