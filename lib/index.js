...
import { resolveAgentDir, resolveAgentCardPath, isDeepAgentDir } from "./utils/markdown.js"
...
import { mlflowAttrs, setSpanAttrs } from "./telemetry/mlflow.js"
...
export default function AgentProtocolAdapter(srv, options = {}) {
  ...
  // Supports four executor patterns:
  //   1. srv.agent = { executor }              — custom executor set in service handler init()
  //   2. srv.agent = { graph }                 — custom LangGraph graph, plugin provides executor wrapper
  //   3. <agentDir>/AGENTS.md exists          — auto-built deepagent (zero-code mode)
  //   4. cds.connect.to("agent-executor")      — CDS service-based executor (default)
  ...
  async function getRequestHandler() {
    ...
    let executor
    if (srv.agent?.executor) {
      executor = srv.agent.executor
    } else if (srv.agent?.graph) {
      const { GraphExecutor } = await import("../srv/graph.js")
      executor = new GraphExecutor(srv.agent.graph, srv, srv.agent)
    } else if (resolved.agentDir && isDeepAgentDir(resolved.agentDir)) {
      const { GraphExecutor } = await import("../srv/graph.js")
      const { createAutoDeepAgent } = await import("./agents/markdown/deep-agent.js")
      // Pass the Promise — GraphExecutor awaits it on first request.
      executor = new GraphExecutor(
        createAutoDeepAgent(srv, resolved.agentDir),
        srv,
        srv.agent ?? {},
      )
    } else {
      const executorService = await cds.connect.to("agent-executor")
  ...
  router.post("/", express.json({ limit: "5mb" }), async (req, res) => {
    ...
    if (span) {
      ...
      if (taskId) span.setAttribute("agent.task.id", taskId)
      if (contextId) span.setAttribute("agent.context.id", contextId)
      // MLflow Databricks: trace correlation on HTTP span
      setSpanAttrs(span, mlflowAttrs("CHAIN", { functionName: `POST /a2a/${srv.name}/` }))
    }
    ...
    if (method === "message/send" || method === "message/stream") {
      ...
      if (text?.length > cds.env.agents.pool.maxIncomingMessageLength) {
        ...
        if (!res.headersSent) {
          res.status(400).json({
            jsonrpc: "2.0",
            // REVISIT: i18n
            error: { code: -32029, message: "The message must not exceed 5000 characters" },
            id: req.body?.id || null,
          })
        }
        return
    ...
    try {
      ...
      if ((method === "message/send" || method === "message/stream") && !taskId) {
        const { default: quotaEnforcerAtStart } =
          await import("./agents/react/nodes/quotaEnforcerAtStart.js")
