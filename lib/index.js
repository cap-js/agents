...
export default function A2AProtocolAdapter(srv, options = {}) {
  ...
  router.use((req, res, next) => {
    // Dev-only: preview push loopback POSTs from CAP outbox arrive unauthenticated
    if (
      !cds.env.profiles?.includes("production") &&
      cds.env.profiles?.includes("development") &&
      req.method === "POST" &&
      req.path.startsWith("/preview/push/")
    ) {
      return next()
    }
  ...
  router.post("/", express.json({ limit: "5mb" }), async (req, res) => {
    ...
    try {
      ...
      if ((method === "message/send" || method === "message/stream") && !taskId) {
        const { default: quotaEnforcerAtStart } = await import(
          "./agents/quota-enforcer-at-start.js"
        )
