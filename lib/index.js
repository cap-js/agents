...
export default function A2AProtocolAdapter(srv, options = {}) {
  ...
  router.use((req, res, next) => {
    ...
    if (!user || user._is_anonymous) {
      if (req.method === "POST") {
        const body = JSON.stringify({
          jsonrpc: "2.0",
          id: req.body?.id ?? null,
          error: { code: -32001, message: cds.i18n.messages.at("UNAUTHORIZED") },
        })
    ...
    if (req.method === "POST") {
      const body = JSON.stringify({
        jsonrpc: "2.0",
        id: req.body?.id ?? null,
        error: { code: -32003, message: cds.i18n.messages.at("FORBIDDEN") },
      })
  ...
  router.post("/", express.json({ limit: "5mb" }), async (req, res) => {
    ...
    if (span) {
      ...
      // Expose OTel trace id so eval helpers (see lib/testing/index.js) can
      // attach LLM-as-judge assessments to the MLflow trace produced by this
      // request. Header is only set when MLflow export is enabled — invisible
      // to non-eval consumers otherwise.
