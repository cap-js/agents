...
export default function A2AProtocolAdapter(srv, options = {}) {
  ...
  //   4. cds.connect.to("a2a-executor")      — CDS service-based executor (default)
  ...
  async function getRequestHandler() {
    ...
    if (srv.a2a?.executor) {
      ...
      executor = new GraphExecutor(createAutoDeepAgent(srv, resolved.agentDir), srv, srv.a2a ?? {})
    } else {
  ...
  router.post("/", express.json({ limit: "5mb" }), async (req, res) => {
    ...
    if (method === "message/send" || method === "message/stream") {
      ...
      if (text?.length > cds.env.a2a.pool.maxIncomingMessageLength) {
        LOG.warn("message to long", {
          task: short(taskId),
          service: srv.name,
        })
        metrics.errorsTotal.add(1, { ...requestAttrs, "a2a.error.code": 400 })
