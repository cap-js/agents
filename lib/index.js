...
export default function A2AProtocolAdapter(srv, options = {}) {
  ...
  const inputCap = agentConfig(srv, "fileIO")?.maxInputFileSizeBytes || 0
  ...
  router.post("/", express.json({ limit: envelopeLimit }), async (req, res) => {
    ...
    if (method === "message/send" || method === "message/stream") {
      ...
      const maxLen = agentConfig(srv, "quota")?.maxIncomingMessageLength
    ...
    try {
      ...
      if ((method === "message/send" || method === "message/stream") && !taskId) {
        ...
        const quotaResult = await quotaEnforcerAtStart(srv)
        if (quotaResult) {
