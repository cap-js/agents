...
/**
 * Open a collection session. Must be called before the execution.
 * Returns a collect() function that retrieves all captured spans and closes the session.
 *
 * @returns {{ collect: () => object[] }}
 */
function _openSession() {
  const id = ++_sessionCounter
  const session = { traceId: null, spans: [] }
...
/**
 * Ensure the processor is registered and open a collection session.
 * @returns {Promise<{ collect: () => object[] }>}
 */
