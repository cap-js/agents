...
export function circuitBreaker(options = cds.env.requires.llm?.circuitBreaker) {
  return ({ fn, context }) =>
    (arg) => {
      const key = context?.uri ?? "default"
      circuitBreakers[key] ??= new CircuitBreaker(options)
...
class CircuitBreaker {
  ...
  #record(success, now) {
    const { rollingCountTimeout } = this.#options
  ...
  #shouldOpen() {
    const { volumeThreshold, errorThresholdPercentage } = this.#options
