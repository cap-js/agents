...
// One breaker per uri::tenantId. Opens after volumeThreshold calls exceed the
...
export function circuitBreaker() {
  return ({ fn, context }) =>
    (arg) => {
      const key = `${context?.uri}::${context?.tenantId ?? "tenant_id"}`
...
class CircuitBreaker {
  ...
  async fire(fn, arg) {
    const now = Date.now()
    if (this.#state === "open") {
      if (now - this.#openedAt < circuitBreakerDefaultOptions.resetTimeout)
    ...
    try {
      ...
      if (!_is4xx(error)) this.#onError(now)
      throw error
    }
  }
  ...
  #record(success, now) {
    const cutoff = now - circuitBreakerDefaultOptions.rollingCountTimeout
    while (this.#window.length && this.#window[0].t < cutoff) this.#window.shift()
    this.#window.push({ success, t: now })
  }

  #shouldOpen() {
    const { volumeThreshold, errorThresholdPercentage } = circuitBreakerDefaultOptions
...
function _is4xx(error) {
  const status = error?.response?.status
  return status != null && `${status}`.startsWith("4")
}
