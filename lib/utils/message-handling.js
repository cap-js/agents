...
export function agentMessage(text, data, metadata) {
 ...
 * Handles both wire shapes (v0.3 kind-based, v1.0 oneOf); returned as-is (no clone/validation).
 * @param {Array} parts
 * @returns {object | undefined}
 */
export function firstDataPart(parts = []) {
  for (const p of parts) {
    // Nullish payloads fall through to the text parser (spec: `data` is always an object).
