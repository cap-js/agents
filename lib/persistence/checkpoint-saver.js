...
class CdsCheckpointSaver extends BaseCheckpointSaver {
  ...
  async getTuple(config) {
    ...
    if (checkpoint_id) {
      row = await SELECT.one
        .from(CHECKPOINTS)
        .where({ thread_id, checkpoint_ns, checkpoint_id, createdBy: cds.context.user.id })
    } else {
      row = await SELECT.one
        .from(CHECKPOINTS)
        .where({ thread_id, checkpoint_ns, createdBy: cds.context.user.id })
    ...
    const persistAll = cds.env.a2a?.persistAllCheckpointWrites === true
   ...
   * Enable cds.env.a2a.persistAllCheckpointWrites for custom graphs with parallel branches.
   */
...
module.exports = { CdsCheckpointSaver }
