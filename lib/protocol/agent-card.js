...
function isPushNotificationsEnabled() {
  const cfg = cds.env.agents?.pushNotifications
...
/**
 * Merge @agent.fileIO MIME types into an agent card when enabled.
 ...
 */
function applyFileIOCapability(card, srv) {
  const cfg = agentConfig(srv, "fileIO")
...
function buildAgentCard({ name, description, entities, actions, url, streaming = false }) {
  ...
  return applyFileIOCapability({
    ...
    capabilities: { streaming, pushNotifications: isPushNotificationsEnabled() },
...
function buildAgentCardFromCardFile(cardMeta, agentMeta, srv, servicePath, streaming = false) {
  ...
  return {
    ...
    capabilities: cardMeta.metadata?.capabilities || {
      streaming,
...
function generateAgentCard(srv, options = {}, resolved = {}) {
  ...
  if (agentCardPath) {
    ...
    if (cardMeta) {
      ...
      return applyFileIOCapability(card, srv)
    }
