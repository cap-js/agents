...
import { activeAgentConfig } from "../../agents/config.js"
import {
  BaseCheckpointSaver,
  WRITES_IDX_MAP,
  copyCheckpoint,
...
const ASSOC_TYPES = new Set(["cds.Association", "cds.Composition"])

function scalarColumnsOf(entityName) {
  return Object.entries(cds.model.definitions[entityName].elements)
    .filter(([, e]) => !ASSOC_TYPES.has(e.type))
    .map(([name]) => name)
}
...
export class CdsCheckpointSaver extends BaseCheckpointSaver {
  ...
  /**
   * Find the checkpoint namespace holding the most recent checkpoint for a
   * thread. LangGraph writes the root graph state under checkpoint_ns=""; but a
   * graph whose final node runs in a nested namespace (e.g. a custom StateGraph
   * with a "tools" node) may only leave state under that nested ns. Callers that
   * recover final state from the checkpoint can use this to locate the right ns
   * when the default "" lookup misses. Returns undefined if the thread has none.
   */
  async latestNamespace(thread_id) {
    if (!thread_id) return undefined
    const row = await SELECT.one
      .from(CHECKPOINTS)
      .columns("checkpoint_ns")
      .where({ thread_id, createdBy: resolveUserId() })
      .orderBy("checkpoint_id desc")
    return row?.checkpoint_ns
  }
  ...
  async getTuple(config) {
    ...
    if (checkpoint_id) {
      row = await SELECT.one
        .from(CHECKPOINTS)
        .columns(...scalarColumnsOf(CHECKPOINTS))
    ...
    if (!row.checkpoint) {
      LOG.warn("Checkpoint row exists but LOB column is missing — treating as miss", {
        thread_id,
    ...
    const persistAll = activeAgentConfig("persistAllCheckpointWrites") === true
    const writesFilter = { thread_id, checkpoint_ns, checkpoint_id: actualCheckpointId }
    if (!persistAll) writesFilter.idx = { "<": 0 }
    const writeRows = await SELECT.from(WRITES)
      .columns(...scalarColumnsOf(WRITES))
      .where(writesFilter)
  ...
  async put(config, checkpoint, metadata) {
    ...
    const task_id = config.configurable?._taskId || cds.context?.["agent.task.id"] || null
    ...
    LOG._trace &&
      LOG.debug("Checkpoint saved", { thread_id, checkpoint_id: checkpoint.id, task_id })
   ...
   * Enable @agent.persistAllCheckpointWrites for custom graphs with parallel branches.
   */
  ...
  async *list(config, options) {
    ...
    while (true) {
      ...
      for (const row of rows) {
        if (!row.checkpoint) {
          LOG.warn("Checkpoint row has missing LOB — skipping", {
            thread_id,
            checkpoint_id: row.checkpoint_id,
          })
          continue
