...
// TTL from @agent.dataRetention, `false`/0 → disabled.
function resolveTtlMs(serviceName) {
  const cfg = agentConfig(serviceName, "dataRetention")
...
export async function triggerCleanup(serviceName) {
  const ttlMs = resolveTtlMs(serviceName)
  if (!ttlMs) {
    LOG.debug(`@agent.dataRetention is not configured. Skipping cleanup of old Tasks.`)
    return
  }
