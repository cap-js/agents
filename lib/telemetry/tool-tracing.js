...
import { scrubToolCallResult } from "../pseudonymize/helpers.js"
    ...
    const invoke = async (span) => {
      ...
      try {
        ...
        if (typeof outputs === "string") {
          ...
          outputs = await scrubToolCallResult({
            carrier: result,
            content: outputs,
            toolName,
            cql: callArgs.cql ?? callArgs.sql,
          })
        }

        if (span) {
          let inputs = args?.args ?? args
          if (inputs && typeof inputs === "object") {
            const scrubbed = {}
            for (const [k, v] of Object.entries(inputs)) {
              scrubbed[k] = typeof v === "string" ? scrubForTrace(v) : v
            }
            inputs = scrubbed
          }
