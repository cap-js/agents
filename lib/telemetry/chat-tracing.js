...
import { scrubForTrace } from "../pseudonymize/scrub.js"
...
/** Resolve LLM provider from cds.env.requires. */
function _resolveProvider() {
  const kind = cds.env.requires?.llm?.kind || "langchain"
  if (kind === "ai-core" || kind === "aicore") return "sap-ai-core"
  return kind.replace(/^llm-/, "")
}
...
function pickModelParams(params) {
...
export function setLLMSpanStartAttrs(
  ...
  if ((cds.env.agents?.mlflow || LOG._debug) && messages) {
    const mlMessages = messages.map((m) => ({
      role: toRole(m),
      content: scrubForTrace(extractText(m.content)),
    }))
    setSpanAttrs(span, mlflowAttrs("LLM", { model, provider, inputs: { messages: mlMessages } }))
    if (LOG._debug) {
      span.setAttribute(
        "gen_ai.input.messages",
        scrubForTrace(JSON.stringify(messages.map((m) => m.content))),
      )
    }
  }
}
...
export function setLLMSpanEndAttrs(span, { model, provider, tokenUsage, outputContent, response }) {
  if (LOG._debug && outputContent) {
    span.setAttribute("gen_ai.output.messages", scrubForTrace(outputContent))
  }
  ...
  if (response) {
    ...
    if (response.toolCalls?.length > 0) {
      span.setAttribute(
        ...
        scrubForTrace(JSON.stringify(response.toolCalls)),
      )
    }
  }
  ...
  if (tokenUsage) {
    ...
    if (cds.env.agents?.mlflow) {
      const scrubbedOutput = outputContent ? scrubForTrace(outputContent) : outputContent
      const mlOpts = { model, provider, tokenUsage: { ...tokenUsage, reasoning_tokens: undefined } }
      if (scrubbedOutput) {
        mlOpts.outputs = {
          choices: [{ message: { role: "assistant", content: scrubbedOutput } }],
        }
      }
  ...
  if (cds.env.agents?.mlflow && outputContent) {
    setSpanAttrs(
      ...
      mlflowAttrs("LLM", {
        ...
        provider,
        outputs: {
          choices: [{ message: { role: "assistant", content: scrubForTrace(outputContent) } }],
        },
      }),
    )
  }
