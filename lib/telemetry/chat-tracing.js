...
export function setLLMSpanStartAttrs(
  ...
  if ((cds.env.agents?.mlflow || LOG._debug) && messages) {
    const mlMessages = messages.map((m) => ({ role: toRole(m), content: scrubForTrace(extractText(m.content)) }))
    setSpanAttrs(span, mlflowAttrs("LLM", { model, provider, inputs: { messages: mlMessages } }))
    if (LOG._debug) {
      span.setAttribute("gen_ai.input.messages", scrubForTrace(JSON.stringify(messages.map((m) => m.content))))
    }
  }
}
...
export function setLLMSpanEndAttrs(span, { model, provider, tokenUsage, outputContent, response }) {
  ...
  if (response) {
    ...
    if (response.toolCalls?.length > 0) {
      span.setAttribute("gen_ai.response.tool_calls", scrubForTrace(JSON.stringify(response.toolCalls)))
    }
  }
  ...
  if (cds.env.agents?.mlflow && outputContent) {
    setSpanAttrs(
      ...
      mlflowAttrs("LLM", {
        model,
        provider,
        outputs: { choices: [{ message: { role: "assistant", content: scrubForTrace(outputContent) } }] },
      }),
    )
  }
}
