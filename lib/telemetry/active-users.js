 ...
 * Groups by agent_service, counts distinct createdBy.
 *
...
async function _computeForTenant(tenant) {
  ...
  const results = await SELECT.from(TASKS)
    .columns("agent_service", "count(distinct createdBy) as userCount")
    .where({ createdAt: { ">=": since } })
    .groupBy("agent_service")
...
function observeActiveUsers(result) {
  for (const { agent_service, userCount, tenant } of _activeUsersData) {
    result.observe(userCount, {
      "sap.tenantId": tenant,
      "a2a.service": agent_service,
    })
  }
}
 ...
 */
function setupActiveUsersMetric() {
  ...
  const em = cds.spawn({ every: ms }, async () => {
    try {
  ...
  // Stop the timer on graceful shutdown — prevents post-teardown ReferenceErrors
  // under Jest, where @cap-js/cds-test calls cds.shutdown() in its afterAll hook.
  cds.on("shutdown", () => clearInterval(em.timer))

  LOG.debug("active_users metric scheduled", { every })
}

module.exports = { setupActiveUsersMetric, computeActiveUsers, observeActiveUsers, parseInterval }
