...
class RoutingSpanProcessor {
  constructor({ url, getAuthHeaders, BatchSpanProcessor, OTLPTraceExporter }) {
    this._url = url
...
/** Module-level singleton — set by setupMlflowExporter(), used by flushMlflowTraces(). */
let _routing = null
 ...
 * No-op when the exporter was never set up.
 */
...
export async function setupMlflowExporter() {
  ...
  try {
    ...
    if (!mlflowEndpoint) {
      LOG.warn(
        "MLflow: credentials missing (DATABRICKS_HOST or MLFLOW_OTLP_ENDPOINT) — export disabled",
      )
      return
    }
