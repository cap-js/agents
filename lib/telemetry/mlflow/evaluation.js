 ...
 * static MLFLOW_TOKEN or unauthenticated (local MLflow).
 * No-op when cds.env.agents.mlflow is falsy or MLFLOW_HOST is not configured.
 */
export async function postMlflowAssessment(traceId, score, rationale, assessmentName) {
  ...
  if (uc) {
    ...
    url = `${host}/api/4.0/mlflow/traces/${encodeURIComponent(ucTablePrefixFull)}/tr-${traceId}/assessments`
    ...
    url = `${host}/api/3.0/mlflow/traces/tr-${traceId}/assessments`
    body = {
      assessment: {
        trace_id: traceId,
        assessment_name: name,
        source: { source_type: "LLM_JUDGE", source_id: judgeModel },
        feedback: { value: score },
        //expectation: { value: "This is a fancy test" },
        rationale: String(rationale),
      },
    }
  }
