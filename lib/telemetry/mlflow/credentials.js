...
/**
 * Shared credential resolution for all MLflow integrations (tracing + evaluation).
 *
 * Config key: cds.env.requires["databricks-mlflow"].credentials
 * Required:   DATABRICKS_HOST
 * Auth:       OAuth (clientid + clientsecret + url) takes precedence over DATABRICKS_TOKEN.
...
export function resolveMlflowCredentials() {
  ...
  const host = (creds.DATABRICKS_HOST || "").replace(/\/$/, "")
  if (!host) return null
...
function _buildAuthHeaderFactory(creds) {
  if (creds.clientid && creds.clientsecret) {
    const tokenUrl = `${(creds.url || creds.DATABRICKS_HOST || "").replace(/\/$/, "")}/oidc/v1/token`
    let cached = null
    let expiresAt = 0
    return async function fetchOAuthHeaders() {
      ...
      if (!res.ok) {
        ...
        throw new Error(`MLflow OAuth token fetch failed: HTTP ${res.status} — ${body.slice(0, 200)}`)
      }
  ...
  if (creds.DATABRICKS_TOKEN) {
    const headers = { Authorization: `Bearer ${creds.DATABRICKS_TOKEN}` }
