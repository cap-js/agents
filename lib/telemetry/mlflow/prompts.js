...
export async function syncPromptVersion(name, template) {
  ...
  try {
    ...
    let version
    if (_extractHash(prompt?.latestVersion?.tags) === hash) {
    ...
    _linkToExperiment(exporter, name, prompt?.tags).catch(() => {})
    return { name, version }
  } catch {
    return null
  }
...
// Append experiment ID to the model-level "_mlflow_experiment_ids" tags so prompt appears in UI prompts tab.
async function _linkToExperiment(exporter, name, promptTags) {
  const experimentId = resolveExperimentId()
  if (!experimentId) return
  const current = promptTags?.find((t) => t.key === "_mlflow_experiment_ids")?.value
  const ids = current
    ? current
        .replace(/^,|,$/g, "")
        .split(",")
