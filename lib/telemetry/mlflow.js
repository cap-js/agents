...
function resolveExperimentId() {
  ...
  const creds = cds.requires?.["databricks-mlflow"]?.credentials // REVISIT: What is this?
...
export class RoutingSpanProcessor {
  constructor({ url, token, ucTableName, BatchSpanProcessor, OTLPTraceExporter }) {
    this._url = url
    this._token = token
  ...
  _getOrCreate(experimentId) {
    ...
    if (this._ucTableName) headers["X-Databricks-UC-Table-Name"] = this._ucTableName
 ...
 *
 * Unity Catalog (Azure Databricks): set UC_CATALOG, UC_SCHEMA, UC_TABLE_PREFIX in credentials
 * to add the X-Databricks-UC-Table-Name header required for UC trace storage.
 * The OTLP endpoint for Azure Databricks (e.g. .../api/2.0/otel/v1/traces) is part
 * of DATABRICKS_HOST or MLFLOW_OTLP_ENDPOINT — no automatic path construction.
 */
export async function setupMlflowExporter() {
  ...
  try {
    ...
    const isDatabricks = host && /databricks\.(net|com)/i.test(host)
    const defaultPath = isDatabricks ? "/api/2.0/otel/v1/traces" : "/v1/traces"
    const mlflowEndpoint =
      creds.MLFLOW_OTLP_ENDPOINT || (host && `${host.replace(/\/$/, "")}${defaultPath}`)
