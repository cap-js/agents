/**
 * MLflow Databricks span attribute helpers for @cap-js/a2a.
 *
 ...
 *
 * All functions return {} when cds.env.a2a.mlflow is falsy (zero overhead).
 *
...
function resolveExperimentId() {
  ...
  const serviceName = cds.context?.["a2a.service"]
  if (serviceName) {
    const definition =
...
export function mlflowTraceAttrs() {
  if (!cds.env.a2a?.mlflow) return {}
  const session = String(cds.context?.["a2a.context.id"] || "")
