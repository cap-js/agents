 ...
 *      (sourced from `@a2a.card` annotation by the caller)
...
function generateAgentCard(srv, options = {}, resolved = {}) {
  ...
  const streaming = srv?.a2a?.streaming ?? cds.env.a2a?.streaming ?? true

  // Priority 1: Explicit agentCardPath (from @a2a.card annotation)
  if (agentCardPath) {
    ...
    if (cardMeta) {
      ...
      LOG.debug("Generated agent card from @a2a.card annotation", {
        service: srv.name,
        path: agentCardPath,
