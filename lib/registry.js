...
const execute = async (invocation) => {
  ...
  const flushSteps = () => {
    const serialized = JSON.stringify(latestSteps)
    if (serialized === sentSteps) return update
    sentSteps = serialized
    update = update
      .then(() => request("updateInvocation", { ID: invocation.ID, steps: latestSteps }))
      .catch((error) => LOG.warn("invocation progress failed", error.message))
    return update
  }
  const onSteps = (steps) => {
    latestSteps = steps
    clearTimeout(updateTimer)
    updateTimer = setTimeout(flushSteps, 300)
    updateTimer.unref()
  }
  try {
    ...
    const previous = { contextId: invocation.contextId || undefined, onSteps }
