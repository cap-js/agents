...
const { agentMessage } = require("../system-prompt")
    ...
    const runWorkflow = async (wfSpan) => {
      ...
      try {
        ...
        if (isResume) {
          ...
          if (wfSpan) wfSpan.setAttribute("a2a.outcome", state)
          if (approved) {
        ...
        metrics.errorsTotal.add(1, { ...mAttrs, "a2a.error.code": "execution_failed" })
