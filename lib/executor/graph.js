...
const { short, audit } = require("../utils")
  ...
  async execute(requestContext, eventBus) {
    ...
    const runWorkflow = async (wfSpan) => {
      ...
      try {
        ...
        cds.spawn(async () => {
          try {
            const updates = { agent_service: serviceName }
            ...
            if (usageState?._totalTokens != null) updates.usage_llm_tokens = usageState._totalTokens
            if (usageState?._totalToolCalls != null)
              updates.usage_tool_calls = usageState._totalToolCalls
