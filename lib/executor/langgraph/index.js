...
module.exports = class LangGraphExecutorService extends cds.Service {
  ...
  async _ensureInitialized(srv) {
    ...
    const promise = (async () => {
      ...
      LOG.info("Initializing", { service: srv.name, tools: toolNames.length, toolNames })
  ...
  async _execute(srv, requestContext, eventBus) {
    ...
    try {
      ...
      LOG.error("✗ failed", { task: short(taskId), service: srv.name, error: err.message })
      LOG.debug("✗ stack", { task: short(taskId), service: srv.name, stack: err.stack })
      eventBus.publish({
        kind: "status-update",
        taskId,
        contextId,
