...
module.exports = class LangGraphExecutorService extends cds.Service {
  ...
  async _ensureInitialized(srv) {
    ...
    const promise = (async () => {
      ...
      const graph = await createAgentGraph(agentState, nodes)

      this._states.set(srv.name, { graph, systemPrompt })
  ...
  async _execute(srv, requestContext, eventBus) {
    ...
    try {
      ...
      const { graph, systemPrompt } = this._states.get(srv.name)

      const { SystemMessage, HumanMessage } = await import("@langchain/core/messages")
      const messages = [new SystemMessage(systemPrompt), new HumanMessage(userText)]

      LOG.info("Running agent graph", { taskId, service: srv.name })
