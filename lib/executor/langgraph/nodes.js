...
function createNodes(model, toolMap) {
  async function agentNode(state, config) {
    ...
    LOG.info("→ LLM", { task, service, messages: msgCount, iteration })
    ...
    LOG.info("← LLM", { task, service, duration, toolCalls: toolCalls.length, tokens })
    if (toolCalls.length > 0) {
      LOG.debug("← LLM tools", { task, service, tools: toolCalls.map((tc) => tc.name) })
    }
    ...
    const results = await Promise.all(
      state.toolCalls.map(async (toolCall) => {
        ...
        try {
          ...
          LOG.debug("✗ tool stack", { task, service, name, stack: err.stack })
