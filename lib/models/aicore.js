...
import { circuitBreaker, retry, timeout } from "../utils/resilience.js"
...
/**
 * Inject timeout, retry, and circuit breaker middleware into opts.
 * Order: each attempt gets its own timeout; only sustained failures open the breaker.
 */
function _withMiddleware(instance, opts) {
  const llmTimeout = ms4(cds.env.agents?.quotas?.maxLLMCallTimeout || "120s")
  const middleware = [timeout(llmTimeout), retry(2), circuitBreaker()]
  return {
    ...opts,
