...
class _InstrumentedOrchestrationClient extends OrchestrationClient {
  constructor(name, options) {
    ...
    // LLM-service params. Without this, AI Core silently applies its own
    // defaults (temperature ≈ 1, verbose model → repeated max_tokens truncation
    // and extra ReAct iterations — see PR #188 review).
    const params = options.params || (deepAgent ? { max_tokens: 4096, temperature: 0 } : undefined)
...
function _withMiddleware(instance, opts) {
  const llmTimeout = ms4(instance.options.timeout || "120s")
  const middleware = [timeout(llmTimeout), circuitBreaker(instance.options.circuitBreaker)]
  return {
    ...opts,
