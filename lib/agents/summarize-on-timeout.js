...
export async function summarizePartialWork({
  ...
  const fallback = approval
    ? `Task paused after ${reason}. Progress may be incomplete. Continue running or stop?`
    : `The task was stopped (${reason}). Some work may have been done — please check the results or retry with a simpler request.`
  ...
  try {
    ...
    let summaryPrompt = approval
      ? `Agent task reached execution limit and is paused. Reason: ${reason}. Write short, precise progress summary so user can decide whether to continue. State completed work and immediate next work. Do not claim work not shown. End with: Continue running or stop? No other questions to the user allowed!`
