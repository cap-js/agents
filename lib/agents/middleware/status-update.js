...
export function beforeModelHook(state, toolTypes) {
  ...
  for (const tm of toolMsgs) {
    ...
    if (tc && toolTypes.has(tc.name)) publishToolCallEnd(tc, tm.content, tm.status === "error")
  }
...
export function afterModelHook(state, toolTypes) {
  ...
  const otherCalls = toolCalls.filter(
    (tc) => tc.name !== "query" && !tc.name?.endsWith("_query") && toolTypes.has(tc.name),
  )
  ...
  for (const tc of toolCalls) {
    if (!toolTypes.has(tc.name)) continue
