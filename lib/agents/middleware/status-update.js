...
/**
 * Reads tool-call visibility config.
 * Config path is TBD — placeholder under cds.env.agents["status-update"].
 */
function toolCallConfig() {
  const cfg = cds.env.agents?.["status-update"] ?? {}
  return {
    args: cfg.args ?? true,
    result: cfg.result ?? true,
  }
}
...
function publishToolCallStart(tc, label, kind) {
  ...
  const { args: showArgs } = toolCallConfig()
...
function publishToolCallEnd(tc, result, isError) {
  ...
  const { args: showArgs, result: showResult } = toolCallConfig()
      ...
      parts: [
        {
          ...
          data: {
            ...
            ...(showArgs && { args: tc.args }),
            ...(showResult && { result: resolvePseudonyms(typeof result === "string" ? result : JSON.stringify(result)) }),
          },
        },
      ],
    },
