    ...
    wrapToolCall: async (request, handler) => {
      ...
      if (toolMessage?.content) {
        ...
        if (decoded && typeof decoded !== "string") {
          ...
          // Plain string (e.g. read_file output) or unparseable — scrub using session mappings.
          const scrubbed = session.scrubText(toolMessage.content)
          if (scrubbed !== toolMessage.content) {
            outMessage = new ToolMessage({ ...toolMessage, content: scrubbed })
          }
        }
      }
