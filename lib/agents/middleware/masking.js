...
import { ensureSession, resolveRemoteMcpTool } from "../../masking/index.js"
...
export default function maskingMiddleware(srv) {
  ...
  return createMiddleware({
    ...
    beforeAgent: async () => {
      if (!needsMasking(srv.name)) {
        cds.context["agent.masking.skip"] = true
        return
      }
      await ensureSession()
    },
    ...
    wrapToolCall: async (request, handler) => {
      ...
      if (!session || cds.context?.["agent.masking.skip"]) return handler(request)
      ...
      if (toolMessage?.content) {
        ...
        if (decoded) {
          const result = await srv.send("pseudonymize", {
            data: decoded,
          ...
          for (const [hash, original] of result?.mappings ?? []) {
            session.remember(original, hash)
          }
