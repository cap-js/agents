...
import { activeAgentConfig, agentConfig } from "../config.js"
 ...
 * Reads limits from @agent.quota at check time (not creation time).
 * Returns array to spread into middleware config.
 */
export async function quotaEnforcerMiddleware(srv) {
  return [
    createMiddleware({
      ...
      afterModel: {
        canJumpTo: ["end"],
        hook: (state) => {
          const pool = (srv ? agentConfig(srv, "quota") : activeAgentConfig("quota")) || {}
