...
export function toolWrapMiddleware() {
  return createMiddleware({
    ...
    wrapToolCall: async (request, handler) => {
      ...
      try {
        LOG._debug && LOG.debug(`tool ${name} called`)
        const result = await handler(request)
        if (ToolMessage.isInstance(result) && result.artifact?.isError === true) {
          result.status = "error"
        }
        if (result?.status === "error")
          LOG._debug && LOG.debug("[tool] error", name, result.content)
        else LOG._debug && LOG.debug("[tool] completed", name)
        return result
      } catch (err) {
        if (isGraphInterrupt(err)) throw err
        LOG._debug && LOG.debug("[tool] error", name, err)
