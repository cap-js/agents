  ...
  return createMiddleware({
    ...
    wrapToolCall: async (request, handler) => {
      ...
      try {
        ...
        if (ToolMessage.isInstance(result) && result.status !== "error" && typeof result.content === "string" && result.content.startsWith("Error:")) {
          result.status = "error"
        }
        if (result?.status === "error")
          LOG.debug("[tool] error", name, result.content)
