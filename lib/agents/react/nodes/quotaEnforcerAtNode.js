...
export default async function quotaEnforcerAtNode(state, config) {
  const pool = cds.env.agent?.pool
  if (!pool) {
    LOG.warn("No quota pool configuration found at cds.env.agent.pool — quota enforcement disabled")
    return "next"
  }
