...
export default async function quotaEnforcerAtStart() {
  ...
  if (!quotas) {
    LOG.debug(
      "No quota configuration found at cds.env.agents.quotas — quota enforcement disabled",
    )
    return null
  }
  ...
  if (quotas.maxTasksPerHourPerUser != null && lastHourTasksThisUser >= quotas.maxTasksPerHourPerUser) {
    return {
      message: cds.i18n.messages.at("QUOTA_TASKS_PER_HOUR_PER_USER", [quotas.maxTasksPerHourPerUser]),
      retryAfter: secondsUntilNextHour(),
    }
  }
