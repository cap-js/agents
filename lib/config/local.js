...
function fromCodex() {
  ...
  try {
    ...
    // minimal TOML parse supporting both single- and double-quoted values
    const tomlStr = (re) => raw.match(re)?.[1]
    let model = tomlStr(/^model\s*=\s*['"]([^'"]+)['"]/m)
    // resolve active provider: explicit model_provider or fall back to "openai"
    let provider = tomlStr(/^model_provider\s*=\s*['"]([^'"]+)['"]/m) ?? "openai"
    let section = raw.match(new RegExp(`\\[model_providers\\.${provider}\\]([\\s\\S]*?)(?=\\n\\[|$)`))?.[1] ?? ""
    const sectionStr = (re) => section.match(re)?.[1]
    let baseUrl = sectionStr(/base_url\s*=\s*['"]([^'"]+)['"]/)
    let envKey = sectionStr(/env_key\s*=\s*['"]([^'"]+)['"]/)
    let bearerToken = sectionStr(/experimental_bearer_token\s*=\s*['"]([^'"]+)['"]/)
    let requiresOpenAIAuth = /requires_openai_auth\s*=\s*true/.test(section)
    let apiKey = (envKey && process.env[envKey]) || bearerToken
    // when requires_openai_auth is set, read the key from ~/.codex/auth.json
    if (!apiKey && requiresOpenAIAuth) apiKey = fromCodexAuth()
    let config = {}
