...
export async function anonymizeUserMessage(requestContext) {
...
async function pseudonymizeText(text, session) {
  ...
  // Accumulate text analysis results on the session so maskingMiddleware.wrapToolCall
  // can persist them into graph state (textAnalysisResults channel) across turns.
  if (session._textAnalysisResults) {
    session._textAnalysisResults = session._textAnalysisResults.concat(rawEntities)
  } else {
    session._textAnalysisResults = rawEntities
  }
