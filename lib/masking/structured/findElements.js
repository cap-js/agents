...
// strictMasking=true: also includes @Common.Masked:false fields (traces must not expose them).
//
...
export function personalDataElements(entityDef, strictMasking = false, model = null) {
  if (!entityDef?.elements) return new Set()
  const result = new Set()
  _collectPaths(entityDef, strictMasking, model, [], result, 0)
  return result
}

function _collectPaths(entityDef, strictMasking, model, prefix, out, depth) {
  if (!entityDef?.elements || depth > 20) return
  for (const [name, el] of Object.entries(entityDef.elements)) {
    if (!strictMasking && el["@Common.Masked"] === false) continue
    ...
    if (resolved?.elements) {
      _collectPaths(resolved, strictMasking, model, path, out, depth + 1)
      ...
      if (itemStruct?.elements) {
        _collectPaths(itemStruct, strictMasking, model, path, out, depth + 1)
...
// Returns the `returns` type node of an action/function.
// Case-insensitive fallback handles tool names normalised by toolName() (lowercased).
export function actionReturnType(model, srv, actionName) {
  const exact = model.definitions?.[`${srv.name}.${actionName}`]
  if (exact) return exact.returns ?? null
  const lower = `${srv.name}.${actionName}`.toLowerCase()
  const key = Object.keys(model.definitions ?? {}).find((k) => k.toLowerCase() === lower)
  return key ? (model.definitions[key].returns ?? null) : null
}
...
export function discoverElementsToBeMasked(model, srv, cql, strictMasking) {
  if (!cql) return new Set()
  return cache(model, `query:${srv.name}:${cql}:${strictMasking}`, () => {
    let cqn
    try {
    ...
    return _discoverFromCqn(model, srv, cqn, strictMasking)
  })
}
...
function _expandPiiPaths(model, srv, entityDef, expandCols, assocPath, strictMasking, out) {
  if (!entityDef || !expandCols) return
  const annotated = personalDataElements(entityDef, strictMasking)
  for (const expandCol of expandCols) {
    if (expandCol.SELECT && expandCol.as) {
      if (_discoverFromCqn(model, srv, expandCol, strictMasking).size > 0)
...
function _resolvePlainRef(col, ctx) {
  const { model, sources, primaryEntity, strictMasking } = ctx
  const ref = col.ref
  const element = ref[ref.length - 1]
  ...
  if (!elDef) return []
  if (!strictMasking && elDef["@Common.Masked"] === false) return []
  ...
  _collectComplexPaths(elDef, strictMasking, model, [outKey], nested, 0)
  return [...nested]
}

// Walk a single complex/arrayed element's type, emitting path arrays for nested PII.
function _collectComplexPaths(elDef, strictMasking, model, prefix, out, depth) {
  if (depth > 20) return
  const resolved = _structOf(elDef, model)
  if (resolved?.elements) {
    for (const [name, child] of Object.entries(resolved.elements)) {
      if (!strictMasking && child["@Common.Masked"] === false) continue
      const path = [...prefix, name]
      if (shouldHash(child)) {
        if (hasPersonalDataAnnotation(child)) out.add(path)
      } else {
        _collectComplexPaths(child, strictMasking, model, path, out, depth + 1)
      }
    }
  } else if (resolved?.items) {
    const itemStruct = _structOf(resolved.items, model)
    if (itemStruct?.elements) {
      _collectComplexPaths(
        { elements: itemStruct.elements },
        strictMasking,
        model,
        prefix,
        out,
...
function _discoverFromCqn(model, srv, cqn, strictMasking) {
  // Top-level SET: union results from every branch.
  if (cqn.SET) {
    const all = new Set()
    for (const branch of cqn.SET.args ?? [])
      for (const f of _discoverFromCqn(model, srv, branch, strictMasking)) all.add(f)
    return all
  }
  ...
  if (from?.SET) {
    ...
    for (const branch of from.SET.args ?? [])
      for (const f of _discoverFromCqn(model, srv, branch, strictMasking)) innerPii.add(f)
  ...
  if (from?.SELECT) {
    const innerPii = _discoverFromCqn(model, srv, from, strictMasking)
  ...
  for (const [alias, { entityDef }] of sources) {
    ...
    annotatedByAlias.set(alias, personalDataElements(entityDef, strictMasking))
  }
  ...
  const anyDeepAnnotated =
    ...
    [...sources].some(
      ([alias, { entityDef }]) =>
        alias !== null && personalDataElements(entityDef, strictMasking, model).size,
    )
  ...
  if (!columns || columns.some((c) => c === "*")) {
    ...
    for (const [alias, { entityDef }] of sources) {
      if (alias === null) continue
      for (const n of personalDataElements(entityDef, strictMasking, model)) all.add(n)
    }
    return all
  }

  const ctx = { model, srv, strictMasking, sources, annotatedByAlias, primaryEntity }
  ...
  for (const col of columns) {
    if (col.expand && col.ref?.length) {
      ...
      if (_discoverFromCqn(model, srv, col, strictMasking).size > 0) resultNames.add(col.as)
      ...
      const bySub =
        !byRef &&
        _extractSubselects(col).some(
          (sub) => _discoverFromCqn(model, srv, sub, strictMasking).size > 0,
        )
