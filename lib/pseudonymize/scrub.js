...
export function scrubForTrace(text) {
  ...
  if (cds.env.agents?.masking?.resolveInTraces) {
    LOG._debug && LOG.debug(`Skipping pseudonymization of text in OTEL spans because "cds.env.agents.masking.resolveInTraces" = true`)
    return text
  }
  ...
  if (session) {
    ...
    LOG._warn && LOG.warn(`Cannot scrub PII from OTEL spans because pseudonymization context is missing on "cds.context"!`)
    return text
  }
}
