  ...
  for (const row of rows) {
    ...
    for (const field of annotatedFields) {
      if (Array.isArray(field)) {
        ...
        if (nested != null)
          pseudonymizeData(nested, new Set([rest.length === 1 ? rest[0] : rest]), session)
      } else {
...
export function pseudonymizeToolResult({ content, toolName, cql, forLlm = true }) {
  ...
  const _session = cds.context?._pseudoSession
  if (!_session) return content
  const srvName = cds.context?.["agent.service"]
  const _srv = cds.services?.[srvName]
  const _model = cds.context?.model ?? _srv?.model
