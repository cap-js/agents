...
export class PseudoSession {
  ...
  static async loadOrCreate(threadId) {
    ...
    try {
      ...
      if (row) {
        // existing must be sorted from small to large hashes, to ensure _originalToHash is correctly setup
        const existing = new Map(
          (row.mappings ?? [])
            .sort((m1, m2) => m1.hash.length - m2.hash.length)
            .map((r) => [r.hash, r.original]),
        )
