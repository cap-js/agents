...
// Cache key includes tenant so different tenants never share in-memory sessions.
// DB isolation is handled by CDS automatically via cds.context.tenant.
...
export class PseudoSession {
  constructor(threadId, seed, existing = new Map()) {
    ...
    // Lazily-built [original, hash] array sorted longest-original-first, so
    // scrubText doesn't re-sort on every call. Invalidated (null) on insert;
    // rebuilt on next scrub. Inserts are far rarer than lookups.
    // resolveText needs no such array: hashes are fixed-shape ("prefix_8hex")
    // and can never be a substring of one another, so order is irrelevant.
