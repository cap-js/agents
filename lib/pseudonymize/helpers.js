...
async function pseudonymizeToolResult({ content, toolName, cql, model, srv, session }) {
  ...
  try {
    ...
    const annotatedFields =
      toolName === "query"
        ? queryEntityElements(model, srv, cql, true)
        : actionReturnElements(model, srv, toolName, true)
    if (!decoded?.data || !annotatedFields.size) return content
...
export async function scrubToolCallResult({
  ...
  if (model.definitions?.[srvName]?.["@agent.masking"] === false) return content

  const hashed = await pseudonymizeToolResult({ content, toolName, cql, model, srv, session })
