...
export function discoverElementsToBeMasked(model, srv, cql, forLlm) {
  ...
  return cache(model, `query:${srv.name}:${cql}:${forLlm}`, () => {
    ...
    // Don't bail early when any explicit column uses a multi-segment path.
    const hasNavigationCol =
      columns && columns.some((c) => Array.isArray(c?.ref) && c.ref.length > 1)
...
export function pseudonymizeToolResult({ content, toolName, cql, model, srv, session }) {
  if (typeof content !== "string" || !content || !session) return content
  try {
    const decoded = decode(content)
    const annotatedFields =
      toolName === "query"
        ? discoverElementsToBeMasked(model, srv, cql, true)
...
const SCRUBBED = Symbol.for("@cap-js/agents:toolResultScrubbed")
export const SESSION_KEY = "_pseudoSession"
...
// Scrub tool result. No-op when no session is active or when invoked a second time
export async function scrubToolCallResult({
  carrier,
  content,
  toolName,
  cql,
  ...
  const session = sessionArg ?? cds.context?.[SESSION_KEY]
  if (typeof content !== "string" || !content || !session) return content

  const cached = carrier && typeof carrier === "object" ? carrier[SCRUBBED] : undefined
  if (cached && cached.source === content) return cached.hashed
  ...
  if (model.definitions?.[srvName]?.["@Common.Masked"] === false) return content

  const hashed = pseudonymizeToolResult({ content, toolName, cql, model, srv, session })
  if (carrier && typeof carrier === "object") {
    try {
      carrier[SCRUBBED] = { source: content, hashed }
    } catch {
      /* frozen carrier */
    }
  }
