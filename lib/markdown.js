...
/**
 * Resolve a path from a `@a2a.directory` / `@a2a.card` annotation against the
 ...
 *
 * Resolution order:
 *   1. `@a2a.directory` annotation (relative to `.cds` source file dir, or absolute)
 *   2. Slugified service-name convention: `<srcDir>/<slugified(srv.name)>/`
 *   3. `undefined`
 */
export function resolveAgentDir(srv) {
  const annotated = resolveAnnotatedPath(srv, srv?.definition?.["@a2a.directory"])
  if (annotated) {
    if (fs.existsSync(annotated)) return annotated
    LOG.warn("@a2a.directory path not found, falling back to convention", {
      service: srv?.name,
      annotation: srv?.definition?.["@a2a.directory"],
      resolved: annotated,
    })
    return undefined
  }
...
/**
 * Resolve an explicit agent-card markdown file via `@a2a.card` annotation.
...
export function resolveAgentCardPath(srv) {
  return resolveAnnotatedPath(srv, srv?.definition?.["@a2a.card"])
}
