...
describe("GraphExecutor - abort/cancellation", () => {
  it("passes signal to graph.invoke via config", async () => {
    let capturedConfig
    const fakeGraph = {
      checkpointer: {},
  ...
  it("cancelTask aborts running execution", async () => {
    const publishedEvents = []
    const fakeGraph = {
      checkpointer: {},
      invoke: async (_input, config) => {
  ...
  it("abortController is cleaned up after execution completes", async () => {
    const fakeGraph = {
      checkpointer: {},
...
describe("GraphExecutor - graceful timeout", () => {
  ...
  it("timeout with checkpointer calls _summarizeOnTimeout", { timeout: 5000 }, async () => {
    const publishedEvents = []
  ...
  it("timeoutGrace is configurable", { timeout: 5000 }, async () => {
    const publishedEvents = []
    let invokeTime
    const fakeGraph = {
      checkpointer: {}, // prevent auto-injection of CdsCheckpointSaver
