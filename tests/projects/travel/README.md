...
- **Declarative external wiring** — No custom JS needed. MCP servers and downstream A2A agents are automatically discovered by annotating the TravelAgentService with `@agent.connect: 'auto'`
...
- **Structured output (DataPart)** — the agent hands a calling agent or program a machine-readable object via a **tailored** `export_itinerary` tool (defined in `travel-agent/srv/travel-agent-service.js`, built on the plugin's `dataPart()` helper), republished as a `data-*` artifact alongside the human-readable text. The plugin deliberately does not ship a generic emit tool — a DataPart is a contract with a specific client, so the consumer defines its own. See the `itinerary-export` skill and `requests.http` request #6. Together with TextPart (`itinerary-summary`) and FilePart (`file-based-planning`), the sample exercises all three A2A Part types.
